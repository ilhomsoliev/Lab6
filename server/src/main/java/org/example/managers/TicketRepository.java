...
import org.example.utility.TicketComparator;
...
*/
    public boolean validateAll() {
        for (var ticket : new ArrayList<>(get())) {
            if (!ticket.validate()) {
                ServerApp.logger.warn("Ticket с id=" + ticket.getId() + " имеет невалидные поля.");
                return false;
            }
        ...
        ServerApp.logger.info("! Загруженные продукты валидны.");
        return true;
    }
